...
import { Users, Download, Plus, Search, SquarePen, ShieldCheck, Briefcase, GraduationCap, Filter, ChevronDown, ChevronLeft, ChevronRight, User, IdCard, X, Calendar, Check } from 'lucide-react';
...
// Sub-componentes de filtrado. Declarados fuera de UsersModule: definirlos
// dentro del render recreaba el componente en cada pintado y reiniciaba su estado.
  ...
  const StatCard = ({ role, label, count, icon: Icon }: any) => {
    ...
    return (
      <button 
        ...
        className={`relative overflow-hidden rounded-xl p-5 text-left transition-all duration-200 border h-full w-full ${theme.bg} ${theme.border} shadow-sm hover:shadow-md ${isActive ? `ring-2 ring-offset-2 ${theme.ring} dark:ring-offset-slate-900` : 'hover:brightness-95'}`}
      >
        <div className="relative z-10 flex flex-col h-full justify-between">
          <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${isActive ? theme.iconBgActive : theme.iconBgInactive} ${isActive ? theme.iconColorActive : theme.iconColorInactive} shadow-sm`}>

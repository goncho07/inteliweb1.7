...
import { 
  Search, Filter, CalendarDays, Edit2, AlertTriangle, CheckCircle2, 
  ExternalLink, Inbox, XCircle, ChevronDown, Eye, X, BookOpen, Clock, 
  Users, ArrowRight, Megaphone, AlertCircle, Info, Send, Printer, 
  MessageCircle, PieChartIcon, ArrowLeft, MessageSquare, LayoutGrid, 
  Calendar as CalendarIcon, ChevronLeft, ChevronRight, MoreVertical 
} from 'lucide-react';
import { PageHeader, containerVariants, KPICard } from '../components/UI';
import { ModuleProps } from '../types';
import { APP_CONFIG, getStudentAvatarUrl } from '../constants';
...
export const CitationsModule: React.FC<ModuleProps> = ({ globalDate }) => {
  ...
  const activeCitation = useMemo(() => {
    return citationsList.find(c => c.id === selectedCitationId) || null;
  }, [selectedCitationId, citationsList]);

  // Set default selection if none selected and list is not empty
  useEffect(() => {
    if (!selectedCitationId && filteredCitations.length > 0) {
      setSelectedCitationId(filteredCitations[0].id);
    }

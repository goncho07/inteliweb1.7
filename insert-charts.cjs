...
const regex = /<div className="bg-\[\#f0f2f5\] dark:bg-\[\#202c33\] h-\[59px\] px-6 flex items-center justify-between shrink-0 border-b border-slate-200 dark:border-slate-800\/60 z-20">([\s\S]*?)<\/div>\s*<\/div>/;

if (content.match(regex)) {
  content = content.replace(regex, replacement);
  fs.writeFileSync('modules/DashboardModule.tsx', content);
  console.log("Restored charts successfully!");
} else {

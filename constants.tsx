...
export const getStudentAvatarUrl = (student: any) => {
  const firstName = student.name.split(' ')[0];
  const isMale = MALE_NAMES.has(firstName);
  
  const hairColors = ['Blonde', 'BlondeGolden', 'Brown', 'BrownDark', 'Black', 'Red'];
  const clothesColors = ['Blue01', 'Blue03', 'Gray01', 'Gray02', 'Heather', 'Red', 'PastelBlue'];
  const skinColors = ['Tanned', 'Pale', 'Light', 'Brown', 'DarkBrown'];
  const accessoriesTypes = ['Blank', 'Blank', 'Blank', 'Kurt', 'Prescription01', 'Prescription02', 'Round', 'Blank'];
  
  // School uniform styles
  const schoolClothes = ['BlazerShirt', 'BlazerSweater', 'CollarSweater'];
  ...
  // Try to use a numeric ID or fallback to hash calculation
  const idValue = student.id ? (parseInt(String(student.id).replace(/\D/g, ''), 10) || hash) : hash;
  const id = idValue + hash; // Combine to ensure more randomness if id is sequential

  const hair = hairColors[id % hairColors.length];
  const clothesColor = clothesColors[(id + 1) % clothesColors.length];
  const skin = skinColors[(id + 2) % skinColors.length];
  const acc = accessoriesTypes[(id + 3) % accessoriesTypes.length];
  const clothe = schoolClothes[(id + 4) % schoolClothes.length];
  
  const top = isMale 
    ? maleTops[(id + 5) % maleTops.length]
    : femaleTops[(id + 5) % femaleTops.length];
